...
const updateCalculations = () => {
    ...
    termsVestingLabel.textContent = `Your: ${optionsPool} options divided in ${vesting} months`;
    termsExerciseInfo.textContent = `Your: ${exercise} months`;

    eduTimePostCliffOptions.textContent = `${ (Math.floor(optionsPool / vesting * cliff)) } options added instantly`;
...
const updatePreTimelineInfo = () => {
    ...
    timeLineLabel.innerHTML = `${ cName } creates <br/> your option plan`;

    document.getElementById('edu-journey-title').textContent = `${cName} Stock Options Journey`;
}
...
eduVsop.addEventListener("click", () => {
    ...
    sectionEduOnboardingSlide7Info.textContent = "It is a win-win: if you do good - your company does good, and otherwise. It makes your work an investment.";
});
